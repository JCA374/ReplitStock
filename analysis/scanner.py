...
# Legacy compatibility - keep these functions for backward compatibility


def legacy_value_momentum_scan(only_watchlist=False):
    """
    Legacy Value & Momentum Strategy scanner (original implementation)
    Kept for compatibility - use value_momentum_scan() for better performance
    """
    ...
    for ticker in stocks_to_scan:
        ...
        stock_data, fundamentals, data_source = scanner.get_data_from_both_dbs(
            ticker, '1d', '1y')
        ...
        near_52w_high = signals.get(
            'near_52w_high', False)  # Near 52-week high

        # 4. Is the stock profitable?
        is_profitable = fundamental_analysis['overall'].get(
            'is_profitable', False)

        # 5. Does it have a reasonable P/E?
        reasonable_pe = fundamental_analysis['overall'].get(
            'reasonable_pe', True)
