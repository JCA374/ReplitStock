...
def add_fix_button():
    """Add a button to fix technical indicators in the scanner tab"""
    if 'scan_results' not in st.session_state or st.session_state.scan_results is None or st.session_state.scan_results.empty:
        return
        
    if st.button("🔧 Fix Technical Indicators", help="Recalculate technical indicators for current scan results"):
    ...
    for i, row in enumerate(df_res.itertuples()):
        ...
        try:
            ...
            if stock_data is None or stock_data.empty:
                ...
                updated_rows.append({col: getattr(row, col) for col in df_res.columns})
                continue
